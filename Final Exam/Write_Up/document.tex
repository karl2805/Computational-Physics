...
The force $F_{EJ}$ between the Earth and Jupiter is given by a modified version of equation 1

\begin{equation}
	F_{EJ} = -\frac{GM_{E}M_{J}}{r_{2}}\uvec{r}
	\label{eq:force_earth_jupiter}
\end{equation}

where $r$ is now the distance between the Earth and Jupiter and $\uvec{r}$ is the unit vector pointing from the Earth to Jupiter.
